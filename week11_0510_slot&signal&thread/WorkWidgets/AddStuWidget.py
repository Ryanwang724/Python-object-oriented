...
class AddStuWidget(QtWidgets.QWidget):
    ...
    def query_button_action(self):
        ...
        send_msg = {'command':command,'parameters':parameters}

        self.send_command = ExecuteSendCommand(client=self.client, send_message=send_msg)
...
class ExecuteSendCommand(QtCore.QThread):
    return_sig = pyqtSignal(str) # return str

    def __init__(self, client:SocketClient, send_message:dict):
        super().__init__()
        self.client = client
        self.send_msg = send_message

    def run(self):
        self.client.send_command(self.send_msg['command'], self.send_msg['parameters'])
