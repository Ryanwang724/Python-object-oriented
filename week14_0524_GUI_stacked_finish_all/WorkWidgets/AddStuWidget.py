...
class AddStuWidget(QtWidgets.QWidget):
    def __init__(self):
        ...
        self.background_image_path = os.path.join(self.this_file_path, '..', 'Image', 'background', 'water.jpg')  # 設定背景圖片
    ...
    def enable_button(self):
        ...
        if sender is self.name_editor_label:
            ...
            if len(self.subject_editor_label.text().strip()) > 0 and \
                len(self.score_editor_label.text().strip()) > 0 and \
                self.subject_editor_label.text() not in self.stu_message['scores']:
